module GeoPing
  ...
  module Version1_0_0
    class Rpc
      ...
      #   "weblogUpdates.extendedPing" => [:obj, {:name => :str, :url => :str, :changes_url => :str, :feed_url => :str, :tag => :str}],
      ...
      def process_raw_input
        ...
        raise RpcArgumentError, "Unknown method #{result[:method_name]}" unless ["weblogUpdates.ping", "weblogUpdates.extendedPing"].include?(result[:method_name])
        @params       = {
      ...
      def minimal_params_from_array(array)
        raise RpcArgumentError, "Do not recognize as a ping #{array.inspect}" unless array.size == 2
        result = {}
      ...
      def extended_params_from_array(array)
        raise RpcArgumentError, "Do not recognize as an extended ping #{array.inspect}" if array.size < 4 || array.size > 5
        result = {}
        [:name, :url, :changes_url, :feed_url].each_with_index do |v,i|
          result[v] = array[i]
        end
